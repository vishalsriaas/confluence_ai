...
RecoveryCallback = Callable[[str], Awaitable[None]]
...
class VoiceSessionRuntime:
    ...
    def add_agent_turn(self, text: object, *, turn_id: str | None = None) -> bool:
        ...
        if key in self.seen_turn_ids:
            return False
        self.seen_turn_ids.add(key)
        recovered = self.recovery_attempted > self.recovery_succeeded
        self._record_response_latency()
        self._last_agent_turn = max(self._last_agent_turn, self._last_user_turn)
        self.turns.append(
            {
                "role": "AGENT",
                "text": clean,
    ...
    def _record_response_latency(self) -> None:
        if self._last_user_monotonic is None or self._last_agent_turn >= self._last_user_turn:
            return
        latency_ms = max(0, int((time.monotonic() - self._last_user_monotonic) * 1000))
        self.response_latencies_ms.append(latency_ms)
        self._last_agent_turn = self._last_user_turn
