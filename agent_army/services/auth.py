...
from agent_army.services.utils import now
...
def require_access(scope: str) -> str:
    ...
    if ":" not in token_value:
        frappe.throw("Missing Agent Army access token", frappe.PermissionError)

    token_key, secret = token_value.split(":", 1)
    if not frappe.db.exists("AI Access Token", token_key):
        frappe.throw("Invalid Agent Army access token", frappe.PermissionError)

    doc = frappe.get_doc("AI Access Token", token_key)
    if not doc.enabled:
        frappe.throw("Agent Army access token is disabled", frappe.PermissionError)
    ...
    if scope and scope not in scopes and "*" not in scopes:
        frappe.throw(f"Agent Army token missing scope: {scope}", frappe.PermissionError)

    if doc.expires_at and frappe.utils.get_datetime(doc.expires_at) < datetime.now():
        frappe.throw("Agent Army access token expired", frappe.PermissionError)
